<script lang="ts" >
	...
	async function loadMessages(channel: CChannel) {
		await channel.load()
		return channel.messages.toReversed()
		...
		{:then guild}
			...
			{:then channel}
				<div class="messages" bind:this={messagesElem}>
					{#await loadMessages(channel) then msgs}
						<MessageList bind:this={ml} />
						{#each getGroups(ghostMessages) as group}
							<MessageGroup author={group.author} messages={group.messages} />
						{/each}
						{onMount(()=>{ml.bulk_load(msgs)})}
					{/await}
				</div>
